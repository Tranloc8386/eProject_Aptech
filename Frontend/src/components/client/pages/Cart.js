...
const Cart = () => {
  ...
  if (cart.length === 0) return (
    <div className="max-w-2xl mx-auto px-4 py-28 text-center">
      ...
      <h2 className="font-display text-2xl font-bold text-[#1a1a1a] mb-2">Giỏ hàng trống</h2>
      <p className="text-[#999] text-sm mb-8">Bạn chưa thêm sản phẩm nào vào giỏ.</p>
      <Link
        to="/products"
    ...
    <div className="max-w-5xl mx-auto px-4 py-10">
      <div className="flex items-center justify-between mb-8">
        <h1 className="font-display text-3xl font-bold text-[#1a1a1a]">
          Giỏ hàng
          <span className="text-base font-normal text-[#999] ml-2 font-sans">({totalQty} sản phẩm)</span>
        </h1>
        <button
          onClick={clearCart}
      ...
      <div className="flex flex-col lg:flex-row gap-7">
        ...
        <div className="flex-1 space-y-3">
          {cart.map((item) => (
            <div key={item.product_id} className="bg-white rounded-2xl p-4 flex gap-4 shadow-sm border border-[#f5f0ea] hover:shadow-md transition-shadow">
        ...
        <div className="lg:w-72 shrink-0">
          <div className="bg-white rounded-2xl p-6 shadow-sm border border-[#f5f0ea] sticky top-24">
            <h3 className="font-semibold text-[#1a1a1a] mb-5 text-base">Tóm tắt đơn hàng</h3>

            <div className="space-y-2.5 text-sm mb-5">
              {cart.map((item) => (
                <div key={item.product_id} className="flex justify-between text-[#777]">
            ...
            <div className="border-t border-[#f0ebe3] pt-4 mb-6">
              ...
              <p className="text-xs text-[#bbb] mt-1.5">Chưa bao gồm phí vận chuyển</p>
            </div>

...
const Products = () => {
  ...
  // Reset form
  const resetForm = () => {
    setFormData({ name: '', price: '', stock: '', category_id: '', status: 'active', image: null });
    ...
    <div className="bg-[#f7f8fc] min-h-screen p-8">
      ...
      <div className="flex justify-between items-end mb-8">
        ...
        <button 
          ...
          className="bg-blue-600 hover:bg-blue-700 text-white px-5 py-3 rounded-xl flex items-center gap-2 font-medium transition"
        >
          <Plus size={18} />
          Thêm sản phẩm
        </button>
      ...
      <div className="grid grid-cols-4 gap-6 mb-8">
        ...
        <div className="bg-white rounded-2xl border border-gray-200 p-6 shadow-sm flex items-center gap-4">
          ...
          <div>
            ...
            <h2 className="text-3xl font-bold text-gray-800">
              ${(products.reduce((sum, p) => sum + (p.price || 0) * (p.stock || 0), 0) / 1000000).toFixed(1)}M
            </h2>
          </div>
        </div>
      </div>
          ...
          <table className="w-full">
            <thead className="bg-gray-50 border-b border-gray-200">
              <tr className="text-left text-xs uppercase text-gray-500">
                ...
                <th className="text-right pr-6">Giá</th>

                <th className="text-right pr-6">Tồn kho</th>
